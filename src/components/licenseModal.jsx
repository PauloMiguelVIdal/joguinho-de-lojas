...
export const LicenseModal = ({ setor, nomeLicença, index }) => {
  ...
  // ── HELPERS ───────────────────────────────────────────────
  const encontrarEdificio = (nome) => {
    for (const s of SETORES_IDS) {
      const idx = dados[s]?.edificios?.findIndex((e) => e.nome === nome);
      if (idx !== undefined && idx !== -1) return { ed: dados[s].edificios[idx], setor: s, idx };
    }
    return null;
  };
  ...
  const custoRecursoRec = (nomeRecurso, visited = new Set()) => {
    ...
    const found = encontrarEdificio(nomeRecurso);
    if (!found) return 0;
    const { ed } = found;
  ...
  const encontrarLicencaDoEdificio = (nomeEd, setorEd) => {
    const licencas = dados[setorEd]?.licençasSetor || [];
    for (let i = 0; i < licencas.length; i++) {
      const lic = licencas[i];
      if (lic.edifíciosLiberados?.includes(nomeEd)) {
        return { licencaNome: lic.nome || `Licença ${i + 1}`, valor: lic.valor || 0, status: lic.status, setorLicenca: setorEd };
      }
    }
    return null;
  };
  ...
  const analiseEdificios = useMemo(() => {
    return (licenca.edifíciosLiberados || []).map((nomeEd) => {
      const found = encontrarEdificio(nomeEd);
      ...
      const jaTemEdificio = (ed.quantidade || 0) > 0;
      if (jaTemEdificio) return { nome: nomeEd, custoTotal: 0, licencasFaltando: [], podePagar: true, jaTemEdificio: true };

      let custoTotal = ed.custoConstrucao || 0;
      const licencasFaltando = [];

      const adicionarLicenca = (nomeReq, setorReq) => {
        const edReq = encontrarEdificio(nomeReq);
        if (!edReq) return;
        const liberado = edReq.ed.licençaLiberado?.liberado ?? true;
        if (!liberado) {
      ...
      const tFalta = Math.max(0, (ed.lojasNecessarias?.terrenos || 0) - (dados.terrenos?.quantidade || 0));
      const pFalta = Math.max(0, (ed.lojasNecessarias?.lojasP || 0) - (dados.lojasP?.quantidade || 0));
      const mFalta = Math.max(0, (ed.lojasNecessarias?.lojasM || 0) - (dados.lojasM?.quantidade || 0));
      const gFalta = Math.max(0, (ed.lojasNecessarias?.lojasG || 0) - (dados.lojasG?.quantidade || 0));
      custoTotal +=
      ...
      (ed.recursoDeConstrução || []).forEach((nomeRec) => {
        const foundRec = encontrarEdificio(nomeRec);
        if (!foundRec) return;
        if ((foundRec.ed.quantidade || 0) === 0) {
      ...
      (ed.construçõesNecessárias || []).forEach((nomeNec) => {
        const foundNec = encontrarEdificio(nomeNec);
        if (!foundNec) return;
        if ((foundNec.ed.quantidade || 0) === 0) {
  ...
  }, [dados, licenca, economiaSetores.saldo]);

  // ── PAINEL DE CUSTO (renderizado abaixo de cada card) ─────
  const PainelCusto = ({ analise }) => {
    if (analise.jaTemEdificio) {
      return (
        <div style={{
          marginTop: 6, borderRadius: 10, padding: "5px 8px",
          background: "rgba(122,255,154,.08)",
          border: "1px solid rgba(122,255,154,.2)",
          display: "flex", alignItems: "center", justifyContent: "center", gap: 5,
        }}>
          <span style={{ fontSize: 10 }}>✅</span>
          <span style={{ fontSize: 9, fontWeight: 700, color: "#7aff9a", letterSpacing: ".06em", textTransform: "uppercase" }}>
            Já possui
          </span>
        </div>
      );
    ...
    return (
      <div style={{
        ...
        border: `1px solid ${analise.podePagar ? setorInfo.cor3 + "55" : "rgba(255,96,96,.3)"}`,
        ...
        <div style={{
          ...
          padding: "5px 8px",
          background: analise.podePagar ? `${setorInfo.cor3}18` : "rgba(255,96,96,.08)",
          borderBottom: analise.licencasFaltando.length > 0 ? "1px solid rgba(255,255,255,.06)" : "none",
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <img src={DolarImg} style={{ height: 10, opacity: 0.7 }} alt="" />
            <span style={{ fontSize: 8.5, fontWeight: 700, color: "rgba(255,255,255,.5)", textTransform: "uppercase", letterSpacing: ".08em" }}>
              Investimento
            </span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
            <div style={{
              ...
              background: analise.podePagar ? "#7aff9a" : "#ff6060",
              boxShadow: `0 0 4px ${analise.podePagar ? "#7aff9a" : "#ff6060"}`,
            }} />
            <span style={{
              fontFamily: "'Rajdhani', sans-serif", fontSize: 13, fontWeight: 800,
              color: analise.podePagar ? "#7aff9a" : "#ff9090",
            }}>
              {formatarNumero(analise.custoTotal)}
            </span>
          </div>
        </div>
        ...
        {analise.licencasFaltando.length > 0 && (
          <div style={{ padding: "4px 6px", display: "flex", flexDirection: "column", gap: 3 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 1 }}>
              ...
              <span style={{ fontSize: 7.5, fontWeight: 700, color: "#FFD966AA", letterSpacing: ".1em", textTransform: "uppercase" }}>
                Licenças necessárias
              </span>
            </div>
            {analise.licencasFaltando.map((lic, li) => {
              const setorLicInfo = SETORES.find((s) => s.id === lic.setorLicenca);
              return (
                <div key={li} style={{
                  ...
                  {/* Cor do setor da licença */}
                  <div style={{
                    width: 8, height: 8, borderRadius: 2, flexShrink: 0,
                    background: setorLicInfo?.cor3 || "#FFD966",
                    border: `1px solid ${setorLicInfo?.cor4 || "#FFD966"}55`,
                  }} />
                  <span style={{ fontSize: 8, color: "rgba(255,255,255,.6)", fontFamily: "'Rajdhani', sans-serif", flex: 1, lineHeight: 1.2 }}>
                    {lic.licencaNome}
                    <br />
                    <span style={{ fontSize: 7, color: "rgba(255,255,255,.3)", textTransform: "uppercase", letterSpacing: ".05em" }}>
                      {lic.setorLicenca}
                    </span>
                  </span>
                  <span style={{ fontSize: 9, fontWeight: 700, color: "#FFD966", fontFamily: "'Rajdhani', sans-serif", flexShrink: 0 }}>
                    {formatarNumero(lic.valor)}
                  </span>
                </div>
              );
            })}
  ...
  const renderCards = () => {
    ...
    // Wrapper individual: card + painel de custo abaixo
    const CardComPainel = ({ nome, i, animProps, style = {} }) => {
      const analise = analiseEdificios.find((a) => a.nome === nome);
      return (
        <motion.div key={nome} {...animProps} style={{ display: "flex", flexDirection: "column", ...style }}>
          {Localizador(nome)}
      ...
      return (
        <div className="flex justify-center items-start h-full">
          <CardComPainel
            nome={eds[0]}
            i={0}
            animProps={{ initial: { scale: 0.85, opacity: 0 }, animate: { scale: 1, opacity: 1 }, transition: { delay: 0.1, type: "spring", stiffness: 120 } }}
        ...
        <div className="flex justify-center items-start gap-4 h-full">
          {eds.map((nome, i) => (
            <CardComPainel
              ...
              nome={nome}
              i={i}
              animProps={{ initial: { scale: 0.8, opacity: 0, x: i === 0 ? -20 : 20 }, animate: { scale: 1, opacity: 1, x: 0 }, transition: { delay: 0.1 + i * 0.08, type: "spring", stiffness: 120 } }}
            />
          ))}
        </div>
      );
        ...
        <div className="flex justify-center items-start gap-3 h-full">
          {eds.map((nome, i) => (
            <CardComPainel
              ...
              nome={nome}
              i={i}
              animProps={{ initial: { scale: 0.75, opacity: 0, y: 20 }, animate: { scale: 1, opacity: 1, y: 0 }, transition: { delay: 0.08 + i * 0.07, type: "spring", stiffness: 120 } }}
      ...
      <div className="flex items-start gap-4 h-full overflow-x-auto pb-1 scrollbar-custom" style={{ scrollbarWidth: "thin" }}>
        {eds.map((nome, i) => (
          <CardComPainel
            ...
            nome={nome}
            i={i}
            animProps={{ initial: { scale: 0.7, opacity: 0, y: 16 }, animate: { scale: 1, opacity: 1, y: 0 }, transition: { delay: 0.06 + i * 0.06, type: "spring", stiffness: 110 } }}
            style={{ transform: "scale(0.85)", transformOrigin: "top center", flexShrink: 0 }}
          />
        ))}
      </div>
    );
    ...
    <div className="w-full pb-[24px]">
      ...
      >
        ...
        <div style={{ display: "flex", minHeight: 0 }}>
          ...
          {/* ÁREA DAS CARTAS — agora com minHeight maior para acomodar painéis */}
          <div style={{
            flex: "0 0 75%", padding: "18px 16px", position: "relative",
            minHeight: qtdCards <= 2 ? 440 : qtdCards <= 3 ? 420 : 440,
          }}>
            ...
            <AnimatePresence>
              {unlockAnim && (
                <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.4 }} transition={{ duration: 0.5 }} style={{ position: "absolute", inset: 0, zIndex: 20, display: "flex", alignItems: "center", justifyContent: "center", background: `radial-gradient(circle, ${setorInfo.cor4}66 0%, transparent 70%)`, pointerEvents: "none", borderRadius: "0 0 0 20px" }}>
          ...
          <div style={{ flex: "0 0 25%", background: `linear-gradient(180deg, ${setorInfo.cor1}CC 0%, ${setorInfo.cor1} 100%)`, borderLeft: `1px solid ${setorInfo.cor3}33`, padding: "16px 14px", display: "flex", flexDirection: "column", gap: 12, overflowY: "auto" }}>
            <div>
              ...
              <p style={{ fontSize: 12, color: "rgba(255,255,255,.75)", lineHeight: 1.65, fontFamily: "'Rajdhani', sans-serif" }}>{licenca.desc}</p>
            </div>
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {(licenca.edifíciosLiberados || []).map((nome) => (
                  <div key={nome} style={{ display: "flex", alignItems: "center", gap: 7, background: jaComprado ? `${setorInfo.cor3}33` : "rgba(255,255,255,.06)", borderRadius: 7, padding: "5px 8px", border: `1px solid ${jaComprado ? setorInfo.cor3 + "55" : "rgba(255,255,255,.08)"}` }}>
                    ...
                    {jaComprado ? <span style={{ marginLeft: "auto", fontSize: 10, color: setorInfo.cor4 }}>✓</span> : <span style={{ marginLeft: "auto", fontSize: 9, color: "rgba(255,255,255,.25)" }}>🔒</span>}
                  </div>
                ))}
              </div>
            </div>
