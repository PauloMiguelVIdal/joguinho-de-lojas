...
function SecaoArmazenamento() {
    ...
    const cats = useMemo(() =>
        ...
        [categorias, getCategoryStorageUI, potentialCapacityByCategory]
    );

    return (
        <Section dot="#FFD966" title="Armazenamento" defaultOpen={false}>
            {/* barra geral */}
                        ...
                        <div key={cat}>
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                                ...
                                <span style={{
                                    ...
                                    color: pct >= 90 ? "#ff4d4d" : pct >= 70 ? "#FFD700" : "rgba(255,255,255,.45)",
                                }}>
                                    {Math.round(usado)}/{total}
                                </span>
                            </div>
                            <Bar value={usado} max={total} color="#FFD966" />
                        </div>
                    ))}
                    {cats.length > 7 && (
