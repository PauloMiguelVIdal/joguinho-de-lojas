...
export const PackOpeningDraft = React.memo(({ onClose, onSorteio }) => {
    ...
    }, [dados, atualizarDados, atualizarDadosProf2, fase, jaSorteouHoje, tipoPacoteAtual, spawnParticles]);
    ...
    useEffect(() => {
        ...
        if (jogoIniciado && diaZero && pacotesNaoAbertos && !modal && fase === "idle" && !aguardandoProximoPacote) {
            const timer = setTimeout(abrirProximoPacoteInicial, 500);
    ...
    // ─── EFFECT: SORTEIO AUTOMÁTICO (dias múltiplos de 30) ──────
    useEffect(() => {
    ...
    }, [dados.dia, dados.ultimoSorteio500, fase, modal, pacotesIniciaisAbertos, aguardandoProximoPacote]);

    // ─── EFFECT: MODAL ABERTO INICIA SORTEIO ─────────────────────
    useEffect(() => {
