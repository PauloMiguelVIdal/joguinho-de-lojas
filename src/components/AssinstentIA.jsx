...
function getEdLiberado(dados, nome) {
  for (const s of SETORES) {
    const ed = dados[s]?.edificios?.find(e => e.nome === nome);
    if (ed) return ed.licençaLiberado?.liberado !== false;
  }
  return true;
}
...
// ════════════════════════════════════════════════════════════
// CÁLCULO FINANCEIRO REAL POR EDIFÍCIO
// faturamentoUnitário × mult_global × mult_setor × qtd
// − impostoFixo (fixo a cada 30d, não varia com economia)
// − impostoSobreFatu × faturamento
// ════════════════════════════════════════════════════════════
function calcFin(ed, econGlobal, econSetor) {
  const qtd  = ed.quantidade || 0;
  if (!qtd) return { fat: 0, impFixo: 0, impFatu: 0, impostos: 0, lucro: 0, margem: 0 };
  const fin   = ed.finanças || {};
  const gM    = getEconomyMult(econGlobal);
  const sM    = getEconomyMult(econSetor);
  const fat   = (fin.faturamentoUnitário || 0) * gM * sM * qtd;   // mensal (já é /dia * ciclo)
  const impFixo = fin.impostoFixo  || 0;                           // fixo por 30d
...
// FLUXO DE PRODUTO
// ════════════════════════════════════════════════════════════
function calcFluxo(dados, pid, prodMap) {
...
// EFICIÊNCIA CONTÍNUA
// ════════════════════════════════════════════════════════════
function calcEfic(ed, fluxosProdutos) {
  const conf = FORMULAS_EDIFICIOS.find(c => c.nomeEdificio === ed.nome);
  if (!conf) return 100;
...
// MOTOR PRINCIPAL
// ════════════════════════════════════════════════════════════
function analisar(dados, graph, econGlobal, econPorSetor) {
  const { prodMap, edMap, armMap } = graph;

  const fluxosProdutos = {};
  Object.keys(prodMap).forEach(pid => {
    const f = calcFluxo(dados, pid, prodMap);
    if (f && f.status !== 'inativo') fluxosProdutos[pid] = f;
  });

  // Edifícios com quantidade > 0
  const edificiosAtivos = [];
  SETORES.forEach(s => {
    dados[s]?.edificios?.forEach(ed => {
      if (!(ed.quantidade > 0)) return;
      const fin = calcFin(ed, econGlobal, econPorSetor?.[s] || 'estável');
      const ef  = calcEfic(ed, fluxosProdutos);
      edificiosAtivos.push({ ...ed, setor: s, ...fin, eficiencia: ef, lucroUnit: fin.lucro / ed.quantidade });
    });
  });
  ...
  SETORES.forEach(s => {
    dados[s]?.edificios?.forEach(ed => {
      if (ed.quantidade > 0 || ed.licençaLiberado?.liberado === false) return;
      edificiosDisponiveis.push({ ...ed, setor: s });
    });
  });
  ...
  // Estoque — capacidade real dos armazéns externos construídos
  const pressaoEstoque = {};
  Object.keys(armMap).forEach(cat => {
    const arms = armMap[cat];
    const capExterno = arms.reduce((s, a) => s + getEdQtd(dados, a.nome) * a.capacidadePorEdificio, 0);
    ...
    // Status baseado na pressão real (não "sem armazém" pois armazém nativo existe sempre)
  ...
  const cadeias = detectarCadeias(dados, graph, fluxosProdutos);
...
// DETECÇÃO DE CADEIAS
// ════════════════════════════════════════════════════════════
function detectarCadeias(dados, graph, fluxosProdutos) {
...
// ════════════════════════════════════════════════════════════
// UI BASE — fontes maiores, scroll interno
// ════════════════════════════════════════════════════════════
const S = {
...
// Scroll container interno de cada view
...
// VISÃO 1 — SAÚDE
// ════════════════════════════════════════════════════════════
function VisaoSaude({ analise, dados }) {
  ...
  const porSetor = useMemo(() => SETORES.map(s => {
    const eds = dados[s]?.edificios?.filter(e => e.quantidade > 0) || [];
    const fat = eds.reduce((sum, e) => sum + (e.finanças?.faturamentoUnitário || 0) * (e.quantidade || 0), 0);
    return { setor: s, count: eds.length, fat };
  }).filter(s => s.count > 0), [dados]);
...
// ════════════════════════════════════════════════════════════
// VISÃO 2 — GARGALOS (com scroll interno)
// ════════════════════════════════════════════════════════════
function VisaoGargalos({ analise, graph }) {
  const { gargalos, fluxosProdutos, cadeias } = analise;
  const { prodMap, edMap } = graph;

  const sangramento = Object.entries(fluxosProdutos)
    .filter(([, f]) => f.status === 'sangramento')
    .map(([pid, f]) => ({ pid, prod: productsCatalog[pid], fluxo: f }))
    .sort((a, b) => Math.abs(b.fluxo.liquido) - Math.abs(a.fluxo.liquido));

  const semVenda = Object.entries(fluxosProdutos)
    .filter(([, f]) => f.status === 'sem_venda')
    .map(([pid, f]) => ({ pid, prod: productsCatalog[pid], fluxo: f }));
  ...
  return (
    <ViewScroll>
      ...
      {!sangramento.length && !semVenda.length && !cadeiasSemVenda.length && (
        <Insight emoji="✅" texto="Nenhum gargalo crítico detectado. Seus fluxos estão saudáveis!" cor="#7aff9a" />
      )}

      {/* Cadeias com quebra visual */}
        ...
        <Card glow="#FFD700">
          ...
          {cadeiasSemVenda.map(c => (
            <div key={c.id} style={{ background: 'rgba(255,215,0,.05)', border: '1px solid rgba(255,215,0,.2)', borderRadius: 10, padding: '10px 12px', marginBottom: 8 }}>
              ...
              {/* Cadeia linear visual */}
              <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 4, marginBottom: 6 }}>
                {c.producao.slice(0, 4).map((edNome, i) => {
                  const s = edMap[edNome]?.setor; const cfg = SETOR_CONFIG[s] || SETOR_CONFIG.industria;
                  return (
                    <React.Fragment key={edNome}>
                      {i > 0 && <span style={{ color: 'rgba(255,255,255,.3)', fontSize: 14 }}>→</span>}
                      <div style={{ background: `${cfg.cor}20`, border: `1px solid ${cfg.cor}50`, borderRadius: 6, padding: '3px 8px', fontSize: 10, color: cfg.cor, fontWeight: 700, fontFamily: "'Rajdhani',sans-serif" }}>{cfg.icon} {edNome}</div>
                    </React.Fragment>
                  );
                })}
                {c.producao.length > 0 && (
                  <>
                    ...
                    <div style={{ background: 'rgba(255,107,107,.15)', border: '1px solid rgba(255,107,107,.4)', borderRadius: 6, padding: '3px 8px', fontSize: 10, color: '#ff6b6b', fontWeight: 800, fontFamily: "'Rajdhani',sans-serif" }}>❌ Sem venda</div>
                  </>
                )}
              </div>
              ...
              return (
                <div key={pid} style={{ background: 'rgba(200,122,255,.08)', border: '1px solid rgba(200,122,255,.25)', borderRadius: 9, padding: '9px 11px', flex: '1 1 130px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 5 }}>
                    ...
                    <div style={{ fontSize: 11, fontWeight: 700, color: '#fff', fontFamily: "'Rajdhani',sans-serif" }}>{prod?.nome || pid}</div>
                  </div>
...
// ════════════════════════════════════════════════════════════
// VISÃO 3 — ECONÔMICO (impostos reais)
// ════════════════════════════════════════════════════════════
function VisaoEconomico({ analise }) {
  ...
  const porSetor = useMemo(() => {
    ...
    edificiosAtivos.forEach(ed => {
      ...
      m[ed.setor].fat      += ed.fat;
      m[ed.setor].impostos += ed.impostos;
      m[ed.setor].lucro    += ed.lucro;
    });
    ...
    <ViewScroll>
      ...
      <Card glow={lucro < 0 ? '#ff6b6b' : '#7aff9a'}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 12 }}>
          {[
            { l: 'Faturamento bruto', v: fmtR(fat),        c: '#7ac8ff' },
            { l: 'Total impostos',   v: `−${fmtR(impostos)}`, c: '#ff6b6b' },
            { l: 'Lucro líquido',    v: fmtR(lucro),       c: lucro >= 0 ? '#7aff9a' : '#ff6b6b' },
          ].map(({ l, v, c }) => (
            <div key={l} style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,.35)', marginBottom: 3 }}>{l}</div>
              <div style={{ fontSize: 16, fontWeight: 900, color: c, fontFamily: "'Rajdhani',sans-serif" }}>{v}</div>
            </div>
          ))}
        </div>
        {/* Breakdown impostos */}
        ...
        <div style={{ textAlign: 'center' }}>
          <span style={S.tag(margem >= 25 ? '#7aff9a' : margem >= 10 ? '#FFD700' : '#ff6b6b')}>Margem líquida: {margem.toFixed(1)}%</span>
        </div>
      </Card>
      ...
      {porSetor.length > 0 && (
        <Card>
          ...
          {porSetor.map(s => {
            const cfg = SETOR_CONFIG[s.setor];
            const maxAbs = Math.max(...porSetor.map(x => Math.abs(x.lucro)), 1);
            const cor = s.lucro >= 0 ? cfg.cor : '#ff6b6b';
            return (
              <div key={s.setor} style={{ marginBottom: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                  <span style={{ fontSize: 11, color: 'rgba(255,255,255,.55)', fontFamily: "'Rajdhani',sans-serif", fontWeight: 700 }}>{cfg.icon} {cfg.label}</span>
                  <span style={{ fontSize: 11, fontWeight: 800, color: cor, fontFamily: "'Rajdhani',sans-serif" }}>{fmtR(s.lucro)} <span style={{ fontSize: 9, color: 'rgba(255,255,255,.3)' }}>({s.fat > 0 ? ((s.lucro / s.fat) * 100).toFixed(0) : 0}%)</span></span>
                </div>
                <div style={{ height: 7, background: 'rgba(255,255,255,.07)', borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${(Math.abs(s.lucro) / maxAbs) * 100}%`, background: cor, borderRadius: 4 }} />
                </div>
              </div>
            );
          })}
      ...
      <Card>
        ...
        {rankingEds.slice(0, 20).map((ed, i) => {
          const cfg  = SETOR_CONFIG[ed.setor] || SETOR_CONFIG.industria;
          const corL = ed.lucro >= 0 ? '#7aff9a' : '#ff6b6b';
          const isOpen = aberto === ed.nome;
          return (
            <div key={ed.nome}>
              <div onClick={() => setAberto(isOpen ? null : ed.nome)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,.05)', cursor: 'pointer' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 3 }}>
                    <span style={{ fontSize: 9, color: cfg.cor }}>{cfg.icon}</span>
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#fff', fontFamily: "'Rajdhani',sans-serif", overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ed.nome}</span>
                    <span style={{ fontSize: 10, color: 'rgba(255,255,255,.35)' }}>×{ed.quantidade}</span>
                  </div>
                  <div style={{ height: 5, background: 'rgba(255,255,255,.06)', borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${(Math.abs(ed.lucro) / maxL) * 100}%`, background: corL, borderRadius: 3 }} />
                  </div>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 800, color: corL, fontFamily: "'Rajdhani',sans-serif" }}>{fmtR(ed.lucro)}</div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,.3)' }}>/mês</div>
                </div>
                <span style={{ fontSize: 10, color: 'rgba(255,255,255,.25)' }}>{isOpen ? '▲' : '▼'}</span>
              </div>
              {isOpen && (
                <div style={{ background: 'rgba(0,0,0,.3)', borderRadius: 8, padding: '9px 11px', margin: '3px 0 6px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6 }}>
                  {[
                    { l: 'Fat. bruto',  v: fmtR(ed.fat),       c: '#7ac8ff' },
                    { l: 'Imp. fixo',  v: fmtR(ed.impFixo),    c: '#ff9090' },
                    { l: 'Imp. fatu.', v: fmtR(ed.impFatu),    c: '#ff6b6b' },
                    { l: 'Impostos',   v: fmtR(ed.impostos),   c: '#ff6b6b' },
                    { l: 'Lucro/un.',  v: fmtR(ed.lucroUnit),  c: ed.lucroUnit >= 0 ? '#7aff9a' : '#ff6b6b' },
                    { l: 'Margem',     v: `${(ed.margem || 0).toFixed(1)}%`, c: ed.margem >= 20 ? '#7aff9a' : '#FFD700' },
                  ].map(({ l, v, c }) => (
                    <div key={l} style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 9, color: 'rgba(255,255,255,.35)' }}>{l}</div>
                      <div style={{ fontSize: 12, fontWeight: 800, color: c, fontFamily: "'Rajdhani',sans-serif" }}>{v}</div>
                    </div>
                  ))}
                </div>
              )}
...
// ════════════════════════════════════════════════════════════
// VISÃO 4 — ESTOQUE (sem "sem armazém" pois armazém nativo existe)
// ════════════════════════════════════════════════════════════
function VisaoEstoque({ analise, dados, graph }) {
  const { pressaoEstoque } = analise;
  const { armMap } = graph;

  const cats = useMemo(() => {
    const ordem = { critico: 0, atencao: 1, baixo: 2, ok: 3 };
    return Object.entries(pressaoEstoque)
      .filter(([, i]) => i.entradaDia > 0)
  ...
  return (
    <ViewScroll>
      ...
      {cats.map(({ cat, capExterno, entradaDia, saidaDia, taxa, diasLotar, diasZerar, status, arms, prods }) => {
        ...
        const prevTexto = capExterno === 0 && entradaDia > 0
          ? '📦 Apenas armazém nativo — considere armazém externo para escalar'
          : acumulando && isFinite(diasLotar)
            ? `⏱ Armazém externo lotará em ~${fmtD(diasLotar)}`
            : !acumulando && saidaDia > entradaDia && isFinite(diasZerar) && capExterno > 0
              ? `⏱ Estoque externo zerado em ~${fmtD(diasZerar)}`
              : acumulando ? '↑ Acumulando' : '✅ Fluxo equilibrado';
        const prevCor = status === 'critico' ? '#ff6b6b' : status === 'atencao' ? '#FFD700' : status === 'baixo' ? '#7ac8ff' : '#7aff9a';

        // Armazéns recomendados (não construídos ou com pouca capacidade)
        const recomendados = arms.filter(a => {
          const qtd = getEdQtd(dados, a.nome);
          return qtd === 0 || (qtd * a.capacidadePorEdificio < entradaDia * 30);
        }).slice(0, 3);
        ...
        return (
          <Card key={cat} style={{ background: cfg.bg, borderColor: `${cfg.cor}35` }} glow={status !== 'ok' ? cfg.cor : undefined}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 9 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: '#fff', fontFamily: "'Rajdhani',sans-serif" }}>{cat}</span>
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,.35)' }}>cap. externa</div>
                <div style={{ fontSize: 15, fontWeight: 800, color: cfg.cor, fontFamily: "'Rajdhani',sans-serif" }}>{capExterno > 0 ? fmt(capExterno) : '—'}</div>
              </div>
            </div>

            {/* Previsão temporal */}
            <div style={{ background: `${prevCor}12`, border: `1px solid ${prevCor}35`, borderRadius: 8, padding: '6px 10px', marginBottom: 9, fontSize: 11, color: prevCor, fontFamily: "'Rajdhani',sans-serif", fontWeight: 700 }}>
              {prevTexto}
            </div>

            {/* Barras de fluxo */}
            {(() => {
              const maxB = Math.max(entradaDia, saidaDia, 0.1);
              return (
                <>
                  <Bar label="Entrada/dia" value={entradaDia} max={maxB} cor="#7ac8ff" right={`+${fmt(entradaDia)}/d`} />
                  <Bar label="Saída/dia"   value={saidaDia}   max={maxB} cor="#F27405" right={`-${fmt(saidaDia)}/d`}  />
                  <div style={{ fontSize: 11, color: taxa > 0 ? '#7ac8ff' : taxa < 0 ? '#F27405' : '#7aff9a', fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, marginBottom: 7 }}>
                    Taxa líquida: {taxa >= 0 ? '+' : ''}{fmt(taxa)}/d
                  </div>
                </>
              );
            })()}
            ...
            {arms.length > 0 && (
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,.4)', marginBottom: 5 }}>ARMAZÉNS PARA ESTA CATEGORIA</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: recomendados.length ? 8 : 0 }}>
                  {arms.slice(0, 4).map(a => {
                    const qtd = getEdQtd(dados, a.nome);
                    return (
                      <div key={a.nome} style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 5, background: qtd > 0 ? 'rgba(100,17,217,.25)' : 'rgba(255,255,255,.05)', color: qtd > 0 ? '#C87AFF' : 'rgba(255,255,255,.35)', border: `1px solid ${qtd > 0 ? '#6411D960' : 'rgba(255,255,255,.1)'}` }}>
                        {qtd > 0 ? `✓ ×${qtd}` : '○'} {a.nome} · {fmt(a.capacidadePorEdificio)}/ed
                      </div>
                    );
                  })}
                </div>
            ...
            {/* Recomendação de construção */}
            {recomendados.length > 0 && (
              <div style={{ background: 'rgba(100,17,217,.12)', border: '1px solid rgba(100,17,217,.3)', borderRadius: 8, padding: '8px 10px' }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: '#C87AFF', marginBottom: 5 }}>🏗️ RECOMENDADOS PARA AMPLIAR CAPACIDADE</div>
                {recomendados.map(a => {
                  const qtd = getEdQtd(dados, a.nome);
                  return (
                    <div key={a.nome} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <div>
                        <span style={{ fontSize: 11, fontWeight: 700, color: '#fff', fontFamily: "'Rajdhani',sans-serif" }}>{a.nome}</span>
                        {qtd > 0 && <span style={{ fontSize: 9, color: '#7aff9a', marginLeft: 6 }}>já tem ×{qtd}</span>}
                      </div>
                      <span style={{ fontSize: 11, fontWeight: 800, color: '#C87AFF', fontFamily: "'Rajdhani',sans-serif" }}>{fmt(a.capacidadePorEdificio)}/ed</span>
                    </div>
                  );
                })}
              </div>
    ...
    <ViewScroll>
      ...
      {setoresAtivos.length > 1 && (
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', margin: '8px 0 12px' }}>
          <button onClick={() => setFiltroSetor(null)} style={{ fontSize: 10, fontWeight: 700, padding: '4px 11px', borderRadius: 6, border: 'none', cursor: 'pointer', background: !filtroSetor ? 'rgba(255,255,255,.22)' : 'rgba(255,255,255,.06)', color: !filtroSetor ? '#fff' : 'rgba(255,255,255,.45)', transition: 'all .15s' }}>Todos</button>
          {setoresAtivos.map(s => { const cfg = SETOR_CONFIG[s]; return <button key={s} onClick={() => setFiltroSetor(filtroSetor === s ? null : s)} style={{ fontSize: 10, fontWeight: 700, padding: '4px 11px', borderRadius: 6, border: 'none', cursor: 'pointer', background: filtroSetor === s ? `${cfg.cor}35` : 'rgba(255,255,255,.06)', color: filtroSetor === s ? cfg.cor : 'rgba(255,255,255,.45)', transition: 'all .15s' }}>{cfg.icon} {cfg.label}</button>; })}
        </div>
      )}
      ...
      <Card>
        ...
        {!filtrados.length
          ...
          : filtrados.map((ed, i) => {
            const cfg  = SETOR_CONFIG[ed.setor] || SETOR_CONFIG.industria;
            const corE = efCor(ed.eficiencia);
            const corL = ed.lucro >= 0 ? '#7aff9a' : '#ff6b6b';
            return (
              <div key={ed.nome} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 0', borderBottom: '1px solid rgba(255,255,255,.05)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ height: 6, background: 'rgba(255,255,255,.06)', borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${ed.eficiencia}%`, background: corE, borderRadius: 3, transition: 'width .5s' }} />
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 10, flexShrink: 0, alignItems: 'center' }}>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 15, fontWeight: 900, color: corE, fontFamily: "'Rajdhani',sans-serif" }}>{ed.eficiencia}%</div>
                    <div style={{ fontSize: 8, color: 'rgba(255,255,255,.3)' }}>efic.</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 12, fontWeight: 800, color: corL, fontFamily: "'Rajdhani',sans-serif" }}>{fmtR(ed.lucroUnit)}</div>
                    <div style={{ fontSize: 9, color: 'rgba(255,255,255,.3)' }}>lucro/un</div>
                  </div>
                </div>
              </div>
            );
...
// ════════════════════════════════════════════════════════════
// VISÃO 6 — OPORTUNIDADES (com scroll interno)
// ════════════════════════════════════════════════════════════
function VisaoOportunidades({ analise, dados, graph, econGlobal, econPorSetor }) {
  ...
  const oportunidades = useMemo(() => {
    ...
    SETORES.forEach(s => dados[s]?.edificios?.forEach(ed => { if (ed.quantidade > 0) (edMap[ed.nome]?.outputs || []).forEach(id => prodAtivos.add(id)); }));
    ...
    edificiosDisponiveis.forEach(ed => {
      ...
      if (confP) {
        confP.formulas.forEach(f => {
          ...
          Object.keys(f.output || {}).forEach(id => { const pm = prodMap[id]; if (!pm) return; if ([...pm.consumidores, ...pm.vendedores].some(n => getEdQtd(dados, n) > 0)) { score += 15; const p = productsCatalog[id]; if (p) motivos.push({ icon: p.icon, nome: p.nome, tipo: 'output' }); } score += Math.min((marketPrices[id] || 0) / 1000, 10); });
        });
      }
      ...
      const fin   = calcFin({ ...ed, quantidade: 1 }, econGlobal, econPorSetor?.[ed.setor] || 'estável');
  ...
  }, [dados, prodMap, edMap, edificiosDisponiveis, econGlobal, econPorSetor]);
  ...
  return (
    <ViewScroll>
                ...
                {c.producao.slice(0, 4).map((edNome, i) => {
                  ...
                  return (
                    <React.Fragment key={edNome}>
                      ...
                      <div style={{ background: `${cfg.cor}20`, border: `1px solid ${cfg.cor}50`, borderRadius: 6, padding: '3px 8px', fontSize: 10, color: cfg.cor, fontWeight: 700 }}>{cfg.icon} {edNome}</div>
                    </React.Fragment>
                  );
                })}
        ...
        <Card glow="#C87AFF">
          ...
          {subexplorados.slice(0, 5).map(({ pid, prod, fluxo, trans }) => (
            <div key={pid} style={{ background: 'rgba(200,122,255,.07)', border: '1px solid rgba(200,122,255,.18)', borderRadius: 10, padding: '10px 12px', marginBottom: 9 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: trans.length ? 7 : 0 }}>
                <span style={{ fontSize: 20 }}>{prod?.icon}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#fff', fontFamily: "'Rajdhani',sans-serif" }}>{prod?.nome}</div>
                  <div style={{ fontSize: 11, color: '#7ac8ff', fontWeight: 700 }}>+{fmt(fluxo.liquido)}/d excedente</div>
                </div>
              </div>
              {trans.slice(0, 2).map(t => (
                <div key={t.edificio} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,.2)', borderRadius: 7, padding: '6px 9px', marginBottom: 4 }}>
                  <span style={{ fontSize: 11, color: SETOR_CONFIG[t.setor]?.cor || '#fff', fontWeight: 700 }}>{SETOR_CONFIG[t.setor]?.icon} {t.edificio}</span>
                  <span style={S.tag('#7aff9a')}>até {fmtR(t.maxOut)}</span>
                </div>
              ))}
            </div>
          ))}
      ...
      {oportunidades.length > 0 && (
        <Card>
          ...
          {oportunidades.map(rec => {
            ...
            const pb  = isFinite(rec.payback) ? rec.payback : null;
            return (
              <div key={rec.nome} style={{ background: `${cfg.cor}08`, border: `1px solid ${cfg.cor}25`, borderRadius: 10, padding: '11px 13px', marginBottom: 9 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 10, fontWeight: 800, color: cfg.cor, textTransform: 'uppercase', marginBottom: 3 }}>{rec.isVenda ? '🏪' : cfg.icon} {cfg.label}</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#fff', fontFamily: "'Rajdhani',sans-serif" }}>{rec.nome}</div>
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontSize: 9, color: 'rgba(255,255,255,.35)' }}>sinergia</div>
                    <div style={{ fontSize: 14, fontWeight: 800, color: cfg.cor, fontFamily: "'Rajdhani',sans-serif" }}>{rec.score.toFixed(0)}</div>
                  </div>
                </div>
                <div style={{ height: 4, background: 'rgba(255,255,255,.07)', borderRadius: 2, overflow: 'hidden', marginBottom: 9 }}>
                  <div style={{ height: '100%', width: `${(rec.score / maxScore) * 100}%`, background: cfg.cor, borderRadius: 2 }} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6, marginBottom: 8, background: 'rgba(0,0,0,.22)', borderRadius: 8, padding: '7px 9px' }}>
                  {[
                    { l: 'Custo',     v: fmtR(rec.custo),          c: 'rgba(255,255,255,.65)' },
                    { l: 'ROI/mês',   v: rec.roi > 0 ? `${rec.roi.toFixed(1)}%` : '—', c: roiCor(rec.roi) },
                    { l: '⏱ Payback', v: pb ? `${pb.toFixed(1)}m` : '—', c: pbCor(rec.payback) },
                  ].map(({ l, v, c }) => (
                    <div key={l} style={{ textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap' }}>
                  {rec.motivos.map((m, i) => <span key={i} style={S.tag(m.tipo === 'input' ? '#7ac8ff' : m.tipo === 'urgente' ? '#ff6b6b' : '#7aff9a')}>{m.icon} {m.nome}</span>)}
                </div>
              </div>
            );
          })}
      ...
      {!oportunidades.length && !subexplorados.length && !cadeiasSemVenda.length && (
        <Insight emoji="🏆" texto="Seu império está bem estruturado! Continue monitorando e expandindo." cor="#7aff9a" />
      )}
    </ViewScroll>
  );
}
...
export default function AssistenteIA() {
  ...
  const analise = useMemo(() => analisar(dados, graph, econGlobal, econPorSetor), [dados, graph, econGlobal, econPorSetor]);
    ...
    <div style={{ width: '100%', height: '100%', background: '#07041a', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', fontFamily: "'Rajdhani','Segoe UI',sans-serif", color: '#fff' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,rgba(76,20,169,.48),rgba(100,17,217,.3))', borderBottom: '1px solid rgba(100,17,217,.38)', padding: '11px 13px 0', flexShrink: 0 }}>
        ...
        <div className="aia-tabs" style={{ display: 'flex', gap: 2, overflowX: 'auto' }}>
          {VIEWS.map(v => {
            const isAtiva = viewAtiva === v.id;
            const badge   = badges[v.id] || 0;
            return (
      ...
      <div className="aia-view" key={viewAtiva} style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        {viewAtiva === 'saude'         && <VisaoSaude         analise={analise} dados={dados} />}
        {viewAtiva === 'gargalos'      && <VisaoGargalos      analise={analise} graph={graph} />}
        {viewAtiva === 'economico'     && <VisaoEconomico     analise={analise} />}
        {viewAtiva === 'estoque'       && <VisaoEstoque       analise={analise} dados={dados} graph={graph} />}
        {viewAtiva === 'eficiencia'    && <VisaoEficiencia    analise={analise} />}
        {viewAtiva === 'oportunidades' && <VisaoOportunidades analise={analise} dados={dados} graph={graph} econGlobal={econGlobal} econPorSetor={econPorSetor} />}
      </div>
    </div>
  );
}