...
// ── CONSTANTES (reutilizadas do PackOpeningDraft) ──────────────────
...
// ── RANKS DOS EDIFÍCIOS (reutilizados) ───────────────────────────
const RankS = [
...
const sortearCartasDoPacotePorRaridade = (tipoPacote, raridadeEspecifica) => {
    ...
    const rankAlvo = rankMapping[raridadeEspecifica.toLowerCase()];
    if (!rankAlvo) {
        console.warn(`⚠️ Raridade "${raridadeEspecifica}" não reconhecida!`);
        return sortearCartasDoPacote(tipoPacote);
    }
    ...
    for (let i = 0; i < quantidades; i++) {
        ...
        if (carta) {
            ...
            cartasSorteadas.push({ nome: carta, rank: rankReal });
        }
    }
    ...
    while (cartasSorteadas.length < quantidades) {
        ...
        if (disponiveis.length > 0) {
            cartasSorteadas.push({ 
                nome: disponiveis[Math.floor(Math.random() * disponiveis.length)], 
                rank: 'C' 
            });
        } else {
            cartasSorteadas.push({ 
...
const sortearCartasDoPacote = (tipoPacote) => {
    ...
    for (let i = 0; i < quantidades; i++) {
        ...
        if (!carta) {
            for (const r of ranksPrioridade) {
                carta = sortearCartaUnicaDoRank(r, usedCards);
                if (carta) { rank = r; break; }
            }
        }
    ...
    while (cartasSorteadas.length < quantidades) {
        ...
        if (pool.length > 0) {
            ...
            cartasSorteadas.push({ nome: pool[idx], rank });
        } else {
            const poolC = RANK_POOLS.C;
            cartasSorteadas.push({ 
                nome: poolC[Math.floor(Math.random() * poolC.length)], 
                rank: 'C' 
            });
        }
    }
...
const gerarUpgrades = (cartas, dados) => {
    ...
    for (const { nome, rank } of cartas) {
        for (const setor of SETORES_ARR) {
            const edificios = dados[setor]?.edificios;
            if (!edificios) continue;
            const idx = edificios.findIndex((ed) => ed.nome === nome);
            if (idx !== -1) {
                upgrades.push({
                    nome,
                    rank,
...
const Pacote = memo(({ fase, onOpen, tema, tipo }) => {
    const packGradient = useMemo(() => `
        linear-gradient(
            160deg, 
            ${tema.cor1} 0%, 
            ${tema.cor2} 25%, 
...
export const PackOpeningWithRarity = React.memo(({ 
    ...
    const tipoPacoteAtual = useMemo(() => {
        if (raridade) {
            ...
            if (PACK_CONFIG[raridadeLower]) {
                return raridadeLower;
            }
            console.warn(`⚠️ Raridade "${raridade}" não reconhecida! Usando "comum" como fallback.`);
        }
        return getTipoPacotePorDia(dados.dia);
    }, [dados.dia, raridade]);
    ...
    return (
        <div style={{
            ...
            {temRaridadeEspecifica && (
                <motion.div
                    ...
                    style={{
                        ...
                        textShadow: `0 0 20px ${temaAtual.cor4}44`,
                    }}
                >
                    ✦ Pacote {raridade.charAt(0).toUpperCase() + raridade.slice(1)} ✦
                </motion.div>
            )}
            ...
            <AnimatePresence>
                {fase !== "revealed" && (
                    <Pacote
                        key="pacote"
                        fase={fase}
                        onOpen={!autoOpen ? handleOpen : undefined}
                        tema={temaAtual}
            ...
            {/* Hint (apenas se não for auto-open) */}
            <AnimatePresence>
                {fase === "idle" && !autoOpen && (
                    <motion.div
            ...
            {/* Botão Abrir (apenas se não for auto-open) */}
            <AnimatePresence>
                {fase === "idle" && !autoOpen && (
                    <motion.button
                        onClick={handleOpen}
