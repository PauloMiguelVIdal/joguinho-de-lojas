...
export default function ManagerPanelInterface({ edificioId, onBack }) {
  ...
  const produtosEstoque = useMemo(() => {
    ...
    edificioConfig.formulas.forEach(formula => {
      Object.keys(formula.input || {}).forEach(id => ids.add(id));
      Object.keys(formula.output || {}).forEach(id => ids.add(id));
    });
    return Array.from(ids).map(id => productsCatalog[id]).filter(Boolean);
  }, [edificioConfig]);

  if (!edificioConfig) return <div className="p-6 text-center text-white">Edifício não encontrado</div>;
